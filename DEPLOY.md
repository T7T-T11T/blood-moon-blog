...
> 当前生产架构：前端（Cloudflare Pages）直连后端（Cloudflare Workers / Hono），数据库为 Supabase（PostgreSQL）。
> 下方架构图反映最新的生产部署形态。`backend/`（Express）仅用于本地开发，不再部署到 Render。
...
> 本地开发架构：前端（Vite dev server）→ Vite proxy `/api` → `backend/`（Express + pg，端口 3000）→ Supabase。
