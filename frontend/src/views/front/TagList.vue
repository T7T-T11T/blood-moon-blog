/** * @file TagList.vue * @description 标签文章列表页（杂志风，与分类页同款横向列表） * * 作用： * -
读取 route.params.slug，展示对应标签下的文章 * - 标签名作为大标题（带 #
前缀），文章以横向列表呈现（非卡片，悬浮主色强调条） * - 底部分页器 * * 数据获取： * -
getArticlesByTag(slug, { page, page_size }) * - 返回 { list, pagination, tag }（tag
字段可选，缺失时用 slug 兜底） * * 路由参数： * - route.params.slug 标签标识 * - slug
变化时自动重新加载 * * 动效（2-3 组）： * - 入场：标签标题 fade-in-up * - 滚动：文章项进入视口时
fade-in-up 错峰（Intersection Observer） * - 悬浮：标题变主色 + 左侧强调条展开 + 阅读箭头右移 */
<template>
  <div ref="rootRef" class="tag-page">
    <!-- ============ 标签标题头部 ============ -->
    <section class="page-header">
      <p class="eyebrow animate-fade-in-down">TAG</p>
    ...
    <div class="content-wrapper">
      ...
      <div v-else class="empty-state">
        <p class="empty-title">该标签下暂无文章</p>
...
async function loadArticles() {
  ...
  try {
    ...
    // 兼容返回结构：data = { list, pagination, tag }
    if (data && data.list) {
      articles.value = data.list;
      total.value = data.pagination?.total ?? 0;
      tag.value = data.tag ?? null;
    } else {
      articles.value = [];
      total.value = 0;
    }
