...
- fix: 系统设置"底部文案"不生效
  - 根因 1：页脚从未渲染 footer_text，设置存了但无处显示
  - 根因 2：设置接口返回下划线键（footer_text/site_name），前端读取驼峰键（footerText/siteName），导致站名/简介/页脚文案均未真正生效（依赖硬编码兜底）
  - 修复：api/settings.js 加载设置时自动将下划线键转驼峰；页脚新增底部文案展示
- chore: 前后端 + workers 版本号统一升至 1.19.3
...
- fix(ui): 文章管理"导出/删除"按钮样式与间距修复
  - 行操作按钮改为 flex 布局统一间距，消除 Element Plus 默认 margin 与布局 gap 叠加导致的按钮间距不一致
  - 工具栏按钮消除双倍间距，操作列宽度调整
- refactor: 移除后台"媒体库"页面（路由/菜单/文件）
  - 上传功能保留：TipTap 编辑器与音乐等上传均通过 api/upload.js 独立调用，不受影响
- chore: 前后端 + workers 版本号统一升至 1.19.1
